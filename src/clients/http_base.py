...
class HttpBase:
    """
    Base HTTP client for API requests with optional auth token
    and Allure logging.
    """
    ...
    def get(self, path: str, params: Dict | None = None, headers: Dict | None = None):
        ...
        with allure.step(f"GET {url}"):
            if params:
                allure.attach(
                    str(params), "Request Params", allure.attachment_type.TEXT
                )
    ...
    def post(self, path: str, json: Dict | None = None, headers: Dict | None = None):
        ...
        with allure.step(f"POST {url}"):
            ...
            try:
                allure.attach(
                    response.text, "Response JSON", allure.attachment_type.JSON
                )
            except:
                pass
