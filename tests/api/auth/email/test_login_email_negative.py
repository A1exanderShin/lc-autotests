...
def test_login_email_empty_values(
    auth_client, session_id_email, password, sessionId, description, assert_response
):
    if sessionId == "valid":
        sessionId = session_id_email

    resp = auth_client.login_email(password=password, sessionId=sessionId)
...
def test_login_email_missing_fields(
    auth_client, session_id_email, payload, description, assert_response
):
...
def test_login_email_invalid_password(
    auth_client, session_id_email, password, description, assert_response
):
    resp = auth_client.login_email(password=password, sessionId=session_id_email)
...
def test_login_email_invalid_sessionId(
    auth_client, session_id_email, sessionId, description, assert_response
):
    resp = auth_client.login_email(password="123123123", sessionId=sessionId)
