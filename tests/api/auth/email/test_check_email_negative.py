...
from tests.fixtures.auth_fixtures import TEST_EMAIL, TEST_IP, TEST_USER_AGENT
...
def test_check_email_missing_fields(auth_client, assert_response, payload, description):
    ...
    assert_response(
        resp, expected=(400, 404), msg=f"Отсутствуют обязательные поля: ({description})"
    )
...
def test_check_email_invalid_email(auth_client, assert_response, email, description):
    resp = auth_client.check_email(email=email, ip=TEST_IP, user_agent=TEST_USER_AGENT)

    assert_response(
        resp, expected=(400, 401, 403), msg=f"Неверный формат email: ({description})"
    )
...
def test_check_email_invalid_ip(auth_client, assert_response, ip, description):
    resp = auth_client.check_email(email=TEST_EMAIL, ip=ip, user_agent=TEST_USER_AGENT)
...
def test_check_email_invalid_user_agent(
    auth_client, assert_response, user_agent, description
):
    resp = auth_client.check_email(email=TEST_EMAIL, ip=TEST_IP, user_agent=user_agent)

    assert_response(
        resp, expected=(200,), msg=f"Неверный формат user-agent: ({description})"
    )
