...
@pytest.mark.parametrize(
    ...
    [
        (
            {"ip": TEST_IP, "user_agent": TEST_USER_AGENT, "platform": TEST_PLATFORM},
            "Нет phone",
        ),
        (
            {
        ...
        (
            {"phone": TEST_PHONE, "ip": TEST_IP, "platform": TEST_PLATFORM},
            "Нет user-agent",
        ),
        (
            {"phone": TEST_PHONE, "ip": TEST_IP, "user_agent": TEST_USER_AGENT},
            "Нет platform",
        ),
        ({}, "Пустой JSON"),
    ],
...
def test_check_phone_invalid_phone(auth_client, assert_response, phone, description):
    ...
    assert_response(
        resp, expected=(400, 401, 500), msg=f"Неверный формат phone: {description}"
    )
...
def test_check_phone_invalid_platform(
    auth_client, assert_response, platform, description
):
    resp = auth_client.check_phone(
...
def test_check_phone_invalid_user_agent(
